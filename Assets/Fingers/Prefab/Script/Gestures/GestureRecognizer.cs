...
namespace DigitalRubyShared
{
    ...
    public class GestureRecognizer : IDisposable, IComparable<GestureRecognizer>
    {
        ...
        private void ResetInternal(bool clearCurrentTrackedTouches)
        {
            ...
            try
            {
                // if this gesture consumed touches, remove them
                foreach (var id in consumedTouches.ToArray())
                {
                    if (currentTrackedTouches.Any(t => t.Id == id))
                    {
                        consumedTouches.Remove(id);
                    }
                }

                // clear tracked touches if asked for
...
#pragma warning restore 0618
        ...
        /// <summary>
        /// Clear all consumed touches
        /// </summary>
        public static void ClearConsumedTouches()
        {
